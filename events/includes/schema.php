...
function eventforge_get_schema_statements(): array
{
    return [
        "
        CREATE TABLE IF NOT EXISTS event_admin_users (
        ...
        "
        CREATE TABLE IF NOT EXISTS eventforge_system (
          id INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
function eventforge_get_schema_seed_statements(): array
{
    return [
        "
...
function eventforge_run_schema_setup(mysqli $connection): void
{
    foreach (eventforge_get_schema_statements() as $sql) {
        if (!mysqli_query($connection, $sql)) {
            throw new RuntimeException('Schema creation failed: ' . mysqli_error($connection));
        }
    }

    foreach (eventforge_get_schema_seed_statements() as $sql) {
        if (!mysqli_query($connection, $sql)) {
            throw new RuntimeException('Schema seed failed: ' . mysqli_error($connection));
        }
    }
}